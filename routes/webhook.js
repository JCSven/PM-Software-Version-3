    ...
    try {
      ...
      try {
        if (!existingCommitmentForGroup) {
          ...
          if (routing.forcedItemType) {
            finalParsed.is_commitment = true;
            finalParsed.item_type = routing.forcedItemType;

            if (!cleanString(finalParsed.task)) {
              finalParsed.task = cleanString(text) || "Unknown task";
            }
          }
          ...
          if (routing.forcedItemType) {
            commitment.item_type = routing.forcedItemType;
            if (!cleanString(commitment.task) || commitment.task === "Unknown task") {
              commitment.task = cleanString(text) || "Unknown task";
            }
          }
