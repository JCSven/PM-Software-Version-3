...
async function downloadTelegramFileToUploads(fileId, fallbackExtension, attachmentType, originalFileName) {
  ...
  const extension = path.extname(originalFileName || telegramFilePath) || fallbackExtension;
...
function matchesTelegramName(actualValue, expectedValues = []) {
  ...
  return expectedValues.some((value) => cleanString(value).toLowerCase() === actual);
}
...
function getTelegramRouting(incoming) {
  ...
  const looksLikeJustinV =
    ...
    matchesTelegramName(fromUsername, ["justinv", "justinv", "jv"]) ||
    matchesTelegramName(fromFullName, ["justin v", "justin vanmeer", "justin van meer"]) ||
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      if ((hasPhoto || hasDocument) && !hasCaptionText && !existingCommitmentForGroup) {
        console.log("Telegram attachment received without caption and no matching media group task. Skipping task creation.");
        return res.sendStatus(200);
      }
      ...
      try {
        ...
        if (hasPhoto) {
          try {
            ...
            const downloadedPhoto = await downloadTelegramPhotoToUploads(photo.file_id);

            if (downloadedPhoto) {
              newAttachments.push(downloadedPhoto);
            }
          } catch (photoError) {
            console.error("Telegram photo download failed:", photoError.message);
          }
        }
        ...
        if (hasDocument) {
          try {
            ...
            console.error("Telegram document download failed:", docError.message);
          }
        }
